...
def compute_relative_trajectory_error_dist(est, gt, delta=1.0):
    """
    Almost the same as t_rte in which the length of a window is one second, 
    while the length of a window in d_rte is one meter (default).

    Args:
        est: the estimated trajectory
        gt: the ground truth trajectory.
        delta: distance window size in meters.

    Returns:
        Relative trajectory error. This is the mean value under the specified distance delta.
    """

    gt_delta_len = np.linalg.norm(
        gt[1:] - gt[:-1], axis=1
    )  # Calculate spatial displacement distance between consecutive frames
    end_index = np.zeros(est.shape[0], dtype=int)

    # calculate where the delta meter endpoint is
    j = 0
    i = 0
    current_sum = 0.0
    while i < est.shape[0]:
        ...
        if j == gt_delta_len.shape[0]:
            ...
            end_index[i] = j + 1
            # Update sum for next i: subtract distance from i to i+1
            current_sum = current_sum - gt_delta_len[i]
            # Since we move i to i+1, but j stays at the end of the window, 
            # we need to ALSO subtract gt_delta_len[j] because it will be 
            # re-added at the start of the next while loop iteration.
            current_sum = current_sum - gt_delta_len[j]
            i = i + 1

    valid_indices = end_index > 0
    if not np.any(valid_indices):
        return 0.0

    # For each window, compute the drift
    # drift = || (est[end] - est[start]) - (gt[end] - gt[start]) ||
    # Which is || est[end] + gt[start] - est[start] - gt[end] ||
    starts = np.where(valid_indices)[0]
    ends = end_index[valid_indices]
    
    errs = est[ends] + gt[starts] - est[starts] - gt[ends]
    d_rtes = np.linalg.norm(errs, axis=1)
...
def compute_relative_trajectory_norm_angle_error_dist(est, gt, delta=1.0):
    ...
    while i < est.shape[0]:
        ...
        if j == gt_delta_len.shape[0]:
            ...
            )  # make sure current_sum < delta now
            current_sum = current_sum - gt_delta_len[i]
            i = i + 1
...
def compute_ate_rte(est, gt, pred_per_min=200):
    """
    ...
    d_rte = compute_relative_trajectory_error_dist(est, gt, delta=1.0)
...
def compute_accruacy_metrics(input_dict, pred_per_min=200, use_local=False):
    gt_pos, pred_pos = (
        input_dict["pos_gt"],
        input_dict["pos_pred"],
    )  # Position of the entire trajectory length

    # Priority for velocity error calculation:
    # 1. Use body-frame velocities if available (best for network performance evaluation)
    # 2. Fallback to global frame velocities (includes orientation drift impact)
    gt_vel = input_dict.get("vel_body_gt")
    pred_vel = input_dict.get("vel_body_pred")

    if gt_vel is None or pred_vel is None:
        gt_vel = input_dict.get("vel_gt")
        pred_vel = input_dict.get("vel_pred")

...
from tartan_imu.utils.constants import DEFAULT_TIMEOUT, MAX_RETRY_ATTEMPTS
...
# Marker the dataloaders embed in the ValueError they raise when a trajectory
# yields no usable IMU windows (e.g. a truncated recording).
_EMPTY_TRAJECTORY_MARKER = "No valid data files found"


def is_empty_trajectory_error(exc: Exception) -> bool:
    """Return ``True`` if ``exc`` marks a trajectory with no usable windows.

    The dataloaders raise :class:`ValueError` carrying
    ``"No valid data files found"`` when a test ``.npz`` produces no valid IMU
    windows. Evaluation uses this predicate to skip that single trajectory
    instead of aborting the whole run.

    Args:
      exc: The exception raised while processing a single trajectory.

    Returns:
      ``True`` for a :class:`ValueError` whose message contains the
      empty-trajectory marker; ``False`` otherwise.
    """
    return isinstance(exc, ValueError) and _EMPTY_TRAJECTORY_MARKER in str(exc)
